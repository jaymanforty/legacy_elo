...
namespace ELO.Modules
{
    public partial class LobbyManagement
    {
        ...
        [Command("QueueTest", RunMode = RunMode.Async)]
        [Alias("Qtest", "lpstest", "listplayerstest", "playerlisttest", "whotest")]
        [Summary("Displays information about the current queue or current game being picked.")]
        public virtual async Task ShowQueueTestAsync()
        {
            using (var db = new Database())
            {
                var lobby = db.GetLobby(Context.Channel);
                ...
                var userList = lobbyQueue.Select(x => db.Players.Find(Context.Guild.Id, x.UserId)).Where(x => x != null).ToList();
                var totalPointsOfLobby = userList.Select(x => x.Points).ToList().Sum();
                ...
                if (lobbyQueue.Count > 0)
                {
                    ...
                    embed.Title = $"{Context.Channel.Name} [{lobbyQueue.Count}/{lobby.PlayersPerTeam * 2}]";
                    embed.Description = $"Game: #{(game?.GameId ?? 0) + 1}\n" +
                                        $"Total Points of Lobby: {totalPointsOfLobby}\n" +
                        string.Join("\n", userList.Select(x => $"{MentionUtils.MentionUser(x.UserId)}" + (GetRankOfPlayer(x,db) != null ? $" -- {MentionUtils.MentionRole(GetRankOfPlayer(x,db).RoleId)}" : "")));
                    await ReplyAsync("", false, embed.Build());
                }
                else
                {
        ...
        public Models.Rank GetRankOfPlayer(Models.Player player, Database db)
        {
